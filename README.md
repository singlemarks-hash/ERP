...
## 1. Firebase 준비 (최초 1회)
1. https://console.firebase.google.com 에서 프로젝트 생성 (기존 payroll 프로젝트 재사용 가능)
2. **빌드 > Authentication > 로그인 방법**에서 `익명` 활성화
3. **빌드 > Firestore Database** 생성 (프로덕션 모드)
4. 프로젝트 설정 > 일반 > 내 앱(웹) 등록 → 발급된 config를 `public/js/firebase-config.js`에 입력
...
firebase login
firebase use <프로젝트ID>
firebase deploy        # Hosting(public/) + Firestore 규칙 배포
```

로컬 미리보기: `firebase emulators:start` 또는 `npx serve public`
